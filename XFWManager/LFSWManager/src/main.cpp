...
ensure_basedir_spec (void)
{
    ...
    newstr = xfce_resource_save_location (XFCE_RESOURCE_CONFIG,
                                       "xfce4" G_DIR_SEPARATOR_S "xfwm4",
                                       FALSE);

    if (g_file_test (newstr, G_FILE_TEST_IS_DIR))
    {
        g_free (newstr);
        return;
    }
    ...
    error = NULL;
    if (!xfce_mkdirhier(newstr, 0700, &error))
    {
        g_warning("Unable to create config dir %s: %s", newstr, error->message);
        g_error_free (error);
        g_free (newstr);
        return;
    }
    ...
    if (g_file_test (old, G_FILE_TEST_EXISTS))
    {
        ...
        newstr = xfce_resource_save_location (XFCE_RESOURCE_CONFIG, path, FALSE);
    ...
    newstr = xfce_resource_save_location (XFCE_RESOURCE_CACHE, "sessions", FALSE);
    if (!xfce_mkdirhier(newstr, 0700, &error))
    {
        g_warning("Unable to create session dir %s: %s", newstr, error->message);
        g_error_free (error);
        g_free (newstr);
        return;
    }
    ...
    if (gdir)
    {
        while ((name = g_dir_read_name (gdir)) != NULL)
        {
            ...
            g_snprintf (path, PATH_MAX, "%s/%s", newstr, name);
            w = fopen (path, "w");
...
main (int argc, char **argv)
{
    ...
    {
#ifdef HAVE_DAEMON
        { "daemon", '\0', 0, G_OPTION_ARG_NONE, (void*)&daemon_mode, N_("Fork to the background"), NULL },
#else
        { "daemon", '\0', 0, G_OPTION_ARG_NONE, (void*)&daemon_mode, N_("Fork to the background (not supported)"), NULL },
#endif
        { "replace", '\0', 0, G_OPTION_ARG_NONE, (void*)&replace_wm, N_("Replace the existing window manager"), NULL },
        { "version", 'V', 0, G_OPTION_ARG_NONE, (void*)&version, N_("Print version information and exit"), NULL },
        { NULL }
    };
  ...
  //  xfce_textdomain (GETTEXT_PACKAGE, PACKAGE_LOCALE_DIR, "UTF-8");
    ...
    status = initialize (0, replace_wm);
    /*
