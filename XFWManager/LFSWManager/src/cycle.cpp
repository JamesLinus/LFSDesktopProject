...
clientSwitchWindow (void)
{
    ...
    newfocus = clientGetPrevious(focus, range | SEARCH_SAME_APPLICATION, WINDOW_REGULAR_FOCUSABLE);
    if (newfocus)
    {
        clientCycleFocusAndRaise (newfocus);
        return TRUE;
    }
    return FALSE;
}
...
clientSwitchApp (void)
{
    ...
    newfocus = clientGetPrevious(focus, range | SEARCH_DIFFERENT_APPLICATION, WINDOW_NORMAL);
    if (newfocus)
    {
        clientCycleFocusAndRaise (newfocus);
        return TRUE;
