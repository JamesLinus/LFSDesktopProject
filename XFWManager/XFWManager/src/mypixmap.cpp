...
find_color(const char *name, XPMColor *colorPtr)
{
    ...
    found = (XPMColorEntry*)bsearch (name, xColors, G_N_ELEMENTS (xColors), sizeof (XPMColorEntry),
...
pixbuf_create_from_xpm (gpointer handle, xfwmColorSymbol *color_sym)
{
    ...
    for (ycnt = 0; ycnt < h; ycnt++)
    {
        ...
        for (n = 0, cnt = 0, xcnt = 0; n < wbytes; n += cpp, xcnt++)
        {
            ...
            color = (XPMColor*)g_hash_table_lookup (color_hash, pixel_str);
...
xfwmPixmapDrawFromGdkPixbuf (xfwmPixmap * pm, GdkPixbuf *pixbuf)
{
    ...
    dest_pixmap = (GdkPixmap*)gdk_xid_table_lookup (pm->pixmap);
    if (dest_pixmap)
    {
    ...
    dest_bitmap = (GdkPixmap*)gdk_xid_table_lookup (pm->mask);
    if (dest_bitmap)
    {
...
xfwmPixmapRenderGdkPixbuf (xfwmPixmap * pm, GdkPixbuf *pixbuf)
{
    ...
    destw = (GdkPixmap*)gdk_xid_table_lookup (pm->pixmap);
    if (destw)
    {
