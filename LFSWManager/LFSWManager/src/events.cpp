...
static eventFilterStatus
handleColormapNotify (DisplayInfo *display_info, myXColormapEvent * ev)
{
    Client *c;
    ...
    c = myDisplayGetClientFromWindow (display_info, ev->window, SEARCH_WINDOW);
    if ((c) && (ev->window == c->window) && (ev->newbool))
    {
        if (c == clientGetFocus ())
        {
            clientInstallColormaps (c);
...
handleEvent (DisplayInfo *display_info, XEvent * ev)
{
    ...
    switch (ev->type)
    {
        ...
        case SelectionClear:
            ...
            break;
        case ColormapNotify:
            handleColormapNotify (display_info, (myXColormapEvent *) ev);
            break;
        case ReparentNotify:
...
xfwm4_event_filter (XEvent * xevent, gpointer data)
{
    ...
    return (eventFilterStatus)(EVENT_FILTER_STOP | status);
}
...
show_window_menu (Client *c, gint px, gint py, guint button, guint32 timestamp)
{
    ...
    frameQueueDraw (c, FALSE);
    y = (gdouble) c->y;
    ops =(MenuOp)(MENU_OP_DELETE | MENU_OP_MINIMIZE_ALL | MENU_OP_WORKSPACES | MENU_OP_MOVE | MENU_OP_RESIZE);
    insensitive = (MenuOp)0;

    if (FLAG_TEST (c->flags, CLIENT_FLAG_MAXIMIZED))
    {
        ops = (MenuOp)(ops|MENU_OP_UNMAXIMIZE);
    }
    else
    {
        ops =(MenuOp)(ops| MENU_OP_MAXIMIZE);
    }

    if (!FLAG_TEST (c->xfwm_flags, XFWM_FLAG_HAS_MOVE))
    {
        insensitive = (MenuOp)(ops|MENU_OP_MOVE);
    }

    if (FLAG_TEST (c->flags, CLIENT_FLAG_ICONIFIED))
    {
        ops =(MenuOp)(ops| MENU_OP_UNMINIMIZE);
    }
    else
    {
        ops =(MenuOp)(ops|MENU_OP_MINIMIZE);
    }

    if (FLAG_TEST (c->flags, CLIENT_FLAG_SHADED))
    {
        ops =(MenuOp)(ops| MENU_OP_UNSHADE);
    }
    else
    {
        ops =(MenuOp)(ops| MENU_OP_SHADE);
    }

    if (FLAG_TEST (c->flags, CLIENT_FLAG_STICKY))
    {
        ops =(MenuOp)(ops| MENU_OP_UNSTICK);
    }
    else
    {
        ops =(MenuOp)(ops| MENU_OP_STICK);
    }

    if (!FLAG_TEST (c->xfwm_flags, XFWM_FLAG_HAS_CLOSE))
    {
        insensitive =(MenuOp)(insensitive| MENU_OP_DELETE);
    }

    if (is_transient || !FLAG_TEST(c->xfwm_flags, XFWM_FLAG_HAS_STICK))
    {
        insensitive =(MenuOp)(insensitive| MENU_OP_STICK | MENU_OP_UNSTICK);
    }

    if (!CLIENT_CAN_HIDE_WINDOW (c))
    {
        insensitive =(MenuOp)(insensitive| MENU_OP_MINIMIZE);
    }

    if (!CLIENT_CAN_MAXIMIZE_WINDOW (c))
    {
        insensitive =(MenuOp)(insensitive| MENU_OP_MAXIMIZE);
    }

    if (!FLAG_TEST (c->xfwm_flags, XFWM_FLAG_HAS_MOVE))
    {
        insensitive =(MenuOp)(insensitive| MENU_OP_MOVE);
    }

    if (!FLAG_TEST_ALL (c->xfwm_flags, XFWM_FLAG_HAS_RESIZE | XFWM_FLAG_IS_RESIZABLE) ||
        FLAG_TEST_ALL (c->flags, CLIENT_FLAG_MAXIMIZED))
    {
        insensitive =(MenuOp)(insensitive| MENU_OP_RESIZE);
    }

    if (FLAG_TEST (c->flags, CLIENT_FLAG_FULLSCREEN))
    {
        insensitive = (MenuOp)(insensitive|MENU_OP_SHADE | MENU_OP_MOVE | MENU_OP_RESIZE | MENU_OP_MAXIMIZE | MENU_OP_UNMAXIMIZE);
    }

    if (FLAG_TEST(c->flags, CLIENT_FLAG_FULLSCREEN))
    {
        ops =(MenuOp)(ops| MENU_OP_UNFULLSCREEN);
    }
    else
    {
        ops =(MenuOp)(ops| MENU_OP_FULLSCREEN);
    }

    if (is_transient || (c->type != WINDOW_NORMAL))
    {
        insensitive =(MenuOp)(insensitive| MENU_OP_FULLSCREEN | MENU_OP_UNFULLSCREEN);
    }

    if (FLAG_TEST(c->flags, CLIENT_FLAG_ABOVE))
    {
        ops =(MenuOp)(ops| MENU_OP_NORMAL | MENU_OP_BELOW);
    }
    else if (FLAG_TEST(c->flags, CLIENT_FLAG_BELOW))
    {
        ops =(MenuOp)(ops| MENU_OP_NORMAL | MENU_OP_ABOVE);
    }
    else
    {
        ops =(MenuOp)(ops| MENU_OP_ABOVE | MENU_OP_BELOW);
    }

    if (is_transient || FLAG_TEST (c->flags, CLIENT_FLAG_FULLSCREEN))
    {
        insensitive =(MenuOp)(insensitive| MENU_OP_NORMAL | MENU_OP_ABOVE | MENU_OP_BELOW);
    }
    ...
    {
        ops =(MenuOp)(ops| MENU_OP_CONTEXT_HELP);
    }
    ...
    {
        insensitive =(MenuOp)(insensitive| MENU_OP_WORKSPACES);
    }
