...
tabwin_widget_get_type (void)
{
    ...
    if (G_UNLIKELY (type == G_TYPE_INVALID))
    {
        ...
        type = g_type_register_static (GTK_TYPE_WINDOW, "Xfwm4TabwinWidget", &info, (GTypeFlags)0);
    }
...
static void
tabwinSetLabel (TabwinWidget *tbw, gchar *classstr, gchar *label, int workspace)
{
    gchar *markup;
    gchar *message;
    PangoLayout *layout;
    ...
    message = pretty_string (classstr);
    markup = g_strconcat ("<span size=\"larger\" weight=\"bold\">", message, "</span>", NULL);
    g_free (message);

    gtk_label_set_markup (GTK_LABEL (tbw->classwidg), markup);
    g_free (markup);
...
tabwinSetSelected (TabwinWidget *tbw, GtkWidget *w)
{
    ...
    tbw->selected_callback = g_signal_connect (G_OBJECT (tbw->selected),
                                               ...
                                               NULL);
    c = (Client*)g_object_get_data (G_OBJECT (tbw->selected), "client-ptr-val");

    if (FLAG_TEST (c->flags, CLIENT_FLAG_ICONIFIED))
    {
        classname = g_strdup_printf ("[ %s ]", c->classhint.res_class);
    }
    else
    {
        classname = g_strdup(c->classhint.res_class);
    }
...
tabwinCreateWidget (Tabwin *tabwin, ScreenInfo *screen_info, gint monitor_num)
{
    ...
    tbw = (TabwinWidget*)g_object_new (tabwin_widget_get_type(), "type", GTK_WINDOW_POPUP, NULL);
    ...
    tbw->classwidg = gtk_label_new ("");
    gtk_label_set_use_markup (GTK_LABEL (tbw->classwidg), TRUE);
    gtk_label_set_justify (GTK_LABEL (tbw->classwidg), GTK_JUSTIFY_CENTER);
    gtk_box_pack_start (GTK_BOX (vbox), tbw->classwidg, TRUE, TRUE, 0);
